...
- 05: sürüyor. Bütçe kuralı uyarınca bölündü: **05a** holder↔post (TR50/M, PH50/M — birincil çizimden tam kaynaklı), **05a-2** kaynaklı kayıtlar + montaj tanığı, **05b** BA2/M taban + bağlantı vidaları + breadboard (engelli: breadboard ürün kimliği sahip kararı; BA2/M→PH50/M vidasının parça numarası/boyu yok).
- 05a: tamamlandı (23 Eylül 2026). Kaynak: TR50/M 0331 rev J, PH50/M 23132 rev B (sahip onayıyla açıldı; dosyalar depoda değil). Ölçüm yöntemi ve eşik ÖLÇMEDEN ÖNCE sabitlendi: 0.05 mm (çizimlerin 0.1 mm çözünürlüğünün yarısı); dış yüzey çevrel yarıçapla, delik yüz düzlemlerinden iç (geçiş) yarıçapla, eksenel konumlar köşe uç değerlerinden. Beklenen değerler envanter olgularından okunur, üreticinin sabitlerinden değil. Tanık `tests/test_support_geometry.py`: main'de 10/24, dalda 24/24 (başparmak vidası TS6H/M çizimi 23136 rev B eklendikten sonra; topuz 7.0→7.9 mm kaynaklı, M6 diş, diş ucu tam delik duvarında — iki çizimin birbirini doğruladığı kontrol de testte). Düzeltilen üretici: holder Ø22.86→Ø25.0, delik Ø13.2(46 mm)→Ø12.8(43.2 mm, 64 kenar ki çokgen 0.05 mm radyal boşluğu yemesin), duvar boyunca kesik kaldırıldı (çizimde yok), başparmak vidası üstten 15.0→12.7 mm, topuz Ø8.9→Ø14.5 ve 10.0 mm dışarıda, render post çapraz deliği 7→10.2 mm; POST_SEAT_MM gerçek 6.8 mm tabana göre (postlar 2.8 mm kısaldı; optikler kıpırdamadı, iz değişmedi). Yolda bulunan ölçüm hataları: yarık yüzleri delik duvarı sanıldı (delik Ø3.58 okundu), boolean deliğin köşeleri yalnız dış yüzeyde kaldığı için çapraz delik görünmedi, yarık kontrolü main'de yarık varken GEÇTİ (köşe tabanlıydı; yüz tabanlı ve yönden bağımsız yapıldı), çıkıntı ölçüsü topuzun eksen dışı kenarını sayıyordu. Envanter düzeltmesi: #78'deki "10.0 mm ayak" okuması yanlıştı, başparmak vidası çıkıntısıdır. Açık: taban/tabla (05b); `validate()` holder üstünü BASE_H=9 ile hesaplıyor, `_post_holder` 12 mm ayak kuruyor (önceden var olan tutarsızlık, bu adımın kapsamı dışında).
- 05a-2: tamamlandı (23 Eylül 2026). `mechanical_library.py`: TR50/M, TR50/M-JP, RS2P/M, PH50/M için kaynaklı şema v1 kayıtları; `PROVENANCE` her değeri envanter olgusuna bağlar, çizim kaynakları okunan dosyanın SHA-256'sını taşır. Tanık `tests/test_support_assembly.py` 26/26: kayıt↔envanter kayması yok; TR50/M uyar, Ø12 mm TR50/M-JP uyar (üretici beyanı), Ø25 mm RS2P/M girmez; oturtulan post Dress Bench'in postuyla aynı yükseklikte (6.8 mm); 10 mm (<12.7) ve 45 mm (>43.2) reddedilir; sıkılı/kilitli vida hareketi tutar; söküm ters sırada. Bulgular: (1) üretici Ø12 mm postların PH holder'lara doğrudan uyduğunu söylüyor — #78'deki 0.1 mm "nominal boşluk" izin verilen boşluk olarak kullanılsaydı motor desteklenen eşleşmeyi reddedecekti; izin verilen boşluk ≥0.8 mm (türetilmiş, muhafazakâr) kaydedildi. (2) Aşama 03 hatası: 12.8−12.0 = 0.8000000000000007, sınırdaki boşluk yuvarlama yüzünden reddediliyordu; 1e-9 mm temsil payı eklendi, tanıkla. (3) Aşama 04b hatası: fastened→seated (gevşetme) yerleştirmeyi insertion 0 ile yeniden çalıştırıyordu — parçayı datumuna ışınlıyor ya da minimum girişi olan delikte gevşetmeyi engelliyordu; yalnız ileri adım yerleştirir, iki varyantın da tanığı var, 04a testindeki habersiz geçici çözüm kaldırıldı.
- 05b: sürüyor. Kaynak edinimi (#81): vidalar üreticinin kendi kit prosedüründen (EDU-SPEB2/M kılavuzu MTN021357-D02 Rev A — taban↔holder M6×10, taban↔tabla M6×16 + pul); tabla MB4560/M (kitin tablası, çizim 6282 rev B: 600×450×12.7, 432×M6×1.0, 25 mm adım, kenardan 12.5 mm; diş derinliği ve diş yönü yok); tabanlar için yayımlanmış ilk tolerans 0.05 mm.
- **Sahip kararları (23 Eylül 2026)** — aşama 05'i kapatmak için:
  1. **Diş yönü:** işaretsiz metrik diş = sağ el (ISO gösterim kuralı; sol el her zaman "LH" ile işaretlenir). Şart: kural benimsenmeden önce doğrulanabilir bir kaynak (standart maddesi ya da üretici beyanı) bulunup kaydedilecek; bulunamazsa benimsenmez ve sahibe söylenir.
  2. **Vida yığını:** şema v2'de bağlantı elemanı ("fastened joint") tipi — kavrama = vida boyu − sıkıştırılan kalınlıklar. Önce tasarım notu, sonra kod; v1 kayıtlar açık bir göçle taşınır, sessizce değişmez.
  3. **Taban:** Dress Bench'in görsel ayağı gerçek tabana çevrilecek — yer varsa BA2/M (50×75), dar yerde BA1/M (25×75), kitin yaptığı gibi. BA1/M çizimi aynı onay türüyle okunacak; çakışma denetimi ve ölçüm tanığı ile.
- Karar 1 uygulandı (23 Eylül 2026): ISO 965-1:1998 §5.4 kaynağı bulundu (resmi örnek, madde alıntılı, SHA-256'lı); `standards_rules` altında kayıtlı; kayıtlar `hand: right` + [çizim, iso965_1]. Motor hâlâ yön varsaymıyor. Karşı-olgusal tanık: aynı gerçek çift yönsüz `thread.hand` eksikliği verir. MB4560/M ve kitin M6×16 vidası kütüphanede; tek bilinmeyen diş derinliği.
- Karar 2 tasarım notu yazıldı: `docs/mechanics/schema-v2-fasteners.md` (kod yok). Özü: fastened joint = baş + geçilen katmanlar + diş; kavrama = vida boyu − Σ katman; eksik değer asla geçmez; yalnız sınırı bilinen değerler için aralık aritmetiği (orta nokta alınmaz); açık `carries`; açık göç. İncelemesi bekleniyor.
- Sıra (her biri ayrı inceleme birimi): diş-yönü kaynağı + kural → şema v2 tasarım notu → BA1/M çizimi + taban geometrisi → şema v2 kodu → tabla/taban/vida kayıtları + tam zincir tanığı (05 kapanır).
- 06–12: bekliyor.
