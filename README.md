...
| lens vs. flat reference → concentric rings (Gaussian wavefront) | AO sensor: injected defocus + astigmatism + coma (RMS 0.56 λ) | after the closed loop → flat (RMS 0.00 λ) |

*All generated by the add-on — a Cycles render, the Scan + Plot operator, the Detector Fringe
Image, a polarizer sweep, the **Newton's Rings** example, and the **adaptive-optics** wavefront
sensor before/after closing the loop (the last three painted live into the bottom-left sensor
window, then exported to PNG).*
