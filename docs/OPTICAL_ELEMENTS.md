...
- **Optical behavior:** Focuses a collimated beam to a LINE (PCX cylinder, f>0) or expands along one axis (PCV cylinder, f<0). Used to circularize elliptical laser-diode beams, form light sheets / line foci, and in anamorphic / 1D beam shaping. Power is ANISOTROPIC: f along the powered axis, infinity along the flat axis. The cylinder path now carries a full transverse Gaussian matrix; the spherical path retains scalar ABCD.
...
- **Add-on mapping:** Implemented with `lens_type='CYLINDRICAL'` and signed `focal_length`. Local X is powered; object roll sets orientation. Coupled Gaussian propagation, elliptical sensor fields, slit/aperture power, profiles and baked envelopes are supported. Existing `aperture_shape`, `clear_aperture` and `aperture_half_y` configure rectangular apertures. See [usage and model limits](DISPERSION_AND_SPECTRA.md).
...
- **Real examples:** Thorlabs BB1-E02 (Ø1", 400-750 nm, Ravg>99%, fused silica, lambda/10 at 633nm, 10-5 scratch-dig); BB1-E01 (350-400 nm UV), BB1-E03 (750-1100 nm NIR), BB1-E04 (1280-1600 nm). Square: BBSQ20-E02. Edmund #34-281 family. AOI design 0-45deg; substrate 6 mm thick.
...
#### Dielectric laser-line / high-power laser mirror (e.g. NB1-K08 at 1064, BB1-E02 used on-line)
...
- **Real examples:** Thorlabs NB1-K08 (1064 nm, 0deg, R>99.9%, LIDT 1 J/cm2 at 10ns); BB1-E02 (used as 532/633 turning); high-power Y2 series; Layertec 45deg laser-line HRs. Edmund TECHSPEC laser-line mirrors (355/532/1064). Typ Ø1", 6 mm fused silica.
...
- **Optical behavior:** Linear retarder, retardance=90deg. Jones = rotated diag(exp(-i*pi/4),exp(+i*pi/4)). Linear at 45deg -> circular; circular -> linear@+/-45deg. Core element for circular-polarization generation and optical isolators.
...
GRATINGS (element_type GRATING): `_diffract` uses the element's local Y grooves and the signed transverse grating vector, including conical incidence. Positive order is along local X for a standard +Z face. Evanescent selected orders terminate; order zero remains specular. See [current behavior, sources and limits](DISPERSION_AND_SPECTRA.md). Remaining gaps: wavelength/blaze-dependent efficiency, transmission gratings and anamorphic beam magnification. Flat `reflectivity` remains a single-order efficiency approximation.
...
PRISMS: There is NO general prism behavior; PRISM_MIRROR is a flat 45-deg internal reflector sharing the MIRROR path. Split the real prisms into THREE behavioral buckets and add a NEW param prism_type:
 ...
 (c) REFRACTIVE DEVIATION/DISPERSION (wedge, equilateral dispersing, Pellin-Broca, anamorphic) -> genuinely NEW transmissive behavior the tracer lacks. WEDGE = small fixed bend delta=(n-1)*alpha about the wedge axis (params wedge_angle_deg, clocking, glass n). DISPERSING_EQUILATERAL / PELLIN_BROCA = wavelength-dependent refractive bend via Snell at two tilted faces using a Sellmeier/Abbe dispersion model (params apex_angle_deg, glass, and for Pellin-Broca a design_wl pinned to 90-deg deviation). ANAMORPHIC = 1-D anisotropic beam magnification — still not implemented. The astigmatic Gaussian matrix introduced for cylindrical lenses provides a state representation, but prism anamorphism needs its own transfer law. KEY: refractive dispersion is NOT the grating equation — it is Snell's law at tilted faces with n(lambda), a new code path.
...
- **Add-on mapping:** Maps DIRECTLY to GRATING. Current tracer uses the signed, groove-oriented grating equation with lines_per_mm + grating_order; evanescent orders do not propagate. NEW param needed: blaze_wl_nm (or blaze_angle_deg) to scale per-order EFFICIENCY (currently power = flat reflectivity for every order — a blazed grating should weight the blaze order high and others low). Groove orientation follows object roll (local Y grooves). The geometry path (REFLECTIVE list) is already correct.
...
- **Key params:** Active area Ø0.4-10 mm (e.g. 75.4 mm2 for PDA100A2), spectral band 320-1100 nm, peak responsivity ~0.65 A/W at 970 nm, CW damage threshold (often unspecified for amplified; saturates electrically first), bandwidth (irrelevant to layout).
...
- **Key params:** Spectral band 800-1800 nm, peak responsivity ~0.85 A/W at 1550 nm, active area Ø1-5 mm, higher dark current (noise, not layout).
- **Add-on mapping:** PHOTODIODE with detector_type='GE'; same NEW params as Si but spec_lo/hi = 800/1800, peak_responsivity ~0.85 at 1550. No new behavior beyond the spectral gate added for Si.
...
- **Key params:** Spectral band 800-1700 nm (ext. to 2600), peak responsivity ~1.05 A/W at 1550 nm, active area Ø0.3-2.0 mm (3.14 mm2 for PDA20CS2), CW damage threshold, switchable gain 1.5 kV/A-4.75 MV/A.
- **Add-on mapping:** PHOTODIODE with detector_type='INGAAS' (or 'INGAAS_EXT'); NEW: spec_lo/hi=800/1700 (or 900/2600), peak_responsivity ~1.05 at 1550. clear_aperture = active-area Ø (often ~1-2 mm, smaller than the beam -> the existing _clip_T aperture truncation already models the vignetting).
...
- **Real examples:** EKSMA KTP-402 / KTP-404 (7x7x9 mm, 90-deg or theta=23.5 deg PM, AR at 1064+532, Type II SHG of 1064 nm). Raicol, Cristal Laser KTP. deff ~ 3.2-3.6 pm/V; transparency 350-3500 nm.
...
- **Real examples:** Thorlabs EO-AM-NR-C1 (MgO:LiNbO3 amplitude, 600-900 nm, Vpi ~205 V at 633 nm, O2 mm aperture, >10 dB extinction); EO-PM-NR-C1 (phase, same family). Qubig / iXblue (NIR/telecom LiNbO3 waveguide EOMs, Vpi ~1-5 V, GHz bandwidth).
